    ...
    */
    ...
    void ParseExpr ()
    {
        ...
        while ( TRUE )
        {
            ...
            AddRegICodeOp(g_iCurrScope, iInstrIndex, REG_CODE_T1);

            // Pop the second operand into _T0

            iInstrIndex = AddICodeInstr ( g_iCurrScope, INSTR_POP );
            AddRegICodeOp(g_iCurrScope, iInstrIndex, REG_CODE_T0);
            ...
            case TOKEN_TYPE_IDENT:
            {
                ...
                if ( pSymbol )
                {
                    ...
                    if ( GetLookAheadChar () == '[' )
                    {
                        ...
                        iInstrIndex = AddICodeInstr ( g_iCurrScope, INSTR_POP );
						AddVarICodeOp(g_iCurrScope, iInstrIndex, g_iTempArrayIndexSymbolIndex);

                        // Push the original identifier onto the stack as an array, indexed
                        // with _T0

                        iInstrIndex = AddICodeInstr ( g_iCurrScope, INSTR_PUSH );
                        AddArrayIndexVarICodeOp ( g_iCurrScope, iInstrIndex, pSymbol->iIndex, g_iTempArrayIndexSymbolIndex);
                    }
                    else
                    {
